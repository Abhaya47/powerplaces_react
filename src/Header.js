...
function Header(){
    ...
    return (
        <div>
            <nav className="relative flex bg-[#333] pl-2">
                <div>
                    <img src={logo} className=" aspect-square w-full m-auto h-12 md:h-20 lg:h-30 pt-0 md:pt-0 lg:pt-0 "/>
                </div>
                <div className="absolute md:hidden right-0 pr-5 text-3xl h-screen items-start justify-center sm:opacity-100 md:relative flex md:w-full m-auto text-white">
                    <button className="text-4xl font-extrabold text-shadow-lg text-shadow-red-100" onClick={()=>{changeHamState()}}> <FontAwesomeIcon icon={showHam? faHamburger:faTimes} /></button>
                </div>
                <div className={`absolute ${changeDisplay} md:block lg:block right-0 top-12 md:top-0 z-50 w-4/6 text-3xl h-screen md:h-full lg:h-full items-start justify-center sm:opacity-100 md:relative flex md:w-full m-auto bg-[#333] md:after:bg-[#333]`}>
